...
class Query:
  ...
  def get_edges_by_relationship(self, relationship_type):
  ...
  def get_nodes_by_property_value(self, property_name, property_value):
  ...
  def get_neighbor_nodes(self, node_id, relationship_type):
    """Retrieves neighbor nodes connected by a specific relationship."""
    neighbors = []
    for edge in self.graph.edges:
      if edge.relationship == relationship_type:
        if edge.source_id == node_id:
          neighbors.append(self.graph.get_node(edge.target_id))
        elif edge.target_id == node_id:
          neighbors.append(self.graph.get_node(edge.source_id))
    return neighbors
